...
namespace EnemyAI
{
	...
	public class StateController : MonoBehaviour
	{
		...
		[HideInInspector] public float blindEngageTime = 30f;       // Time to keep targeting last seen position after target leaves sight.
		 public bool targetInSight;                // Is target on sight?
		 public bool focusSight;                   // Will focus on sight position?
		...
		[HideInInspector] public bool hadClearShot;                 // The NPC had a clear sight of the target to shoot before?
		 public bool haveClearShot;                // The NPC has a clear sight of the target to shoot now?
		...
		void Awake()
		{
			aimTarget = GameObject.FindWithTag("MainCamera").transform;
		...
		public void Start()
		{
			...
			enemyAnimation.gunMuzzle = weapon;
			_enemyHealth.weapon = weapon;
		}
