...
public class PlayerShooting : MonoBehaviour
{
	...
	public GameMode gameMode = GameMode.WINDOWNS;
    ...
    void Update()
    {
	    switch (gameMode)
	    {
		    case GameMode.WINDOWNS:
			    ...
			    break;
		    case GameMode.VR:
			    if(weaponMode == WeaponMode.SEMI && OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetDown(OVRInput.Button.Three) || Input.GetMouseButtonDown(0) && canShot)
			    {
				    Debug.Log("SHOOT");
				    Shoot();

			    }
			    else if(weaponMode == WeaponMode.AUTO && OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetDown(OVRInput.Button.Three) || Input.GetMouseButtonDown(0) && canShot)
			    {
				    Shoot();
			    }
			    break;
